...
class MemCellExtractor:
    ...
    EPISODE_PATTERNS = [
        ...
        (
            r"\b(i|we)\s+(went|go|going)\s+(?:to\s+)?(?:the\s+|a\s+|an\s+)?(.+?)(?:\.|,|!|\?|$)",
            "activity",
        ),
        # "went to the beach/park/etc." with location
        (
            r"\b(i|we)\s+(went)\s+to\s+(?:the\s+)?(\w+(?:\s+\w+)?)(?:\s+(?:recently|yesterday|last|this|with))?",
            "visit",
        ),
        # Activities with "attended"
        (
            r"\b(i|we)\s+(attended|attend|attending)\s+(?:the\s+|a\s+|an\s+)?(.+?)(?:\.|,|!|\?|$)",
            "attended",
        ),
        # Activities with "joined"
        (
            r"\b(i|we)\s+(joined|join|joining)\s+(?:the\s+|a\s+|an\s+)?(.+?)(?:\.|,|!|\?|$)",
            "joined",
        ),
        ...
        (
            r"\b(i|we)\s+(painted|drew|made|created|read|finished|bought|got|received|visited|saw|watched)\s+(.+?)(?:\.|,|!|\?|$)",
            "created",
        ),
        # Past activities
        ...
        (
            r"\b(kids?|children|family)\s+(had|have)\s+(?:a\s+|such\s+a\s+)?(\w+(?:\s+\w+)?)(?:\.|,|!|\?|$)",
            "family_activity",
        ),
    ]
    ...
    FACT_PATTERNS = [
        ...
        (
            r"\b(i have|i\'ve got)\s+"
            r"(?:a\s+|an\s+|two\s+|three\s+"
            r"|lots of\s+|many\s+)?"
            r"(.+?)(?:\.|,|!|\?|$)",
            "possession",
        ),
        ...
        (
            r"\bi\s+(collect|have collected|like to collect)\s+(.+?)(?:\.|,|!|\?|$)",
            "hobby_collection",
        ),
    ]
    ...
    RELATION_PATTERNS = [
        ...
        (
            r"\bmy\s+(friend|brother|sister|husband|wife|mom|dad|mother|father|son|daughter|cousin|aunt|uncle)\s+(\w+)",
            "family_friend",
        ),
        # "X is my friend"
        (
            r"\b(\w+)\s+is\s+my\s+(friend|brother|sister|husband|wife|colleague|coworker)",
            "relation",
        ),
    ...
    def extract_from_message(
        ...
        cells.extend(
            self._extract_episodes(
                speaker, content, content_lower, session_id, session_date, session_idx, event_date
            )
        )
        cells.extend(
            self._extract_facts(
                speaker, content, content_lower, session_id, session_date, session_idx
            )
        )
        cells.extend(
        ...
        if use_llm and self.llm_client:
            llm_cells = self._extract_with_llm(
                speaker, content, session_id, session_date, session_idx
            )
    ...
    def _extract_event_date(self, content_lower: str, session_date: str) -> str | None:
        ...
        for pattern, fact_type in self.FACT_PATTERNS:
            for match in re.finditer(pattern, content_lower):
                ...
                if fact_type == "identity":
                    ...
                    fact_content = (
                        f"{speaker} has lots of {groups[1] if len(groups) > 1 else groups[0]}"
                    )
                elif fact_type == "hobby_collection":
                    fact_content = (
                        f"{speaker} collects {groups[1] if len(groups) > 1 else groups[0]}"
                    )
    ...
    def _extract_plans(
        ...
        for pattern, plan_type in self.PLAN_PATTERNS:
            for match in re.finditer(pattern, content_lower):
                ...
                plan_content = (
                    f"{speaker} is planning/wants to {groups[-1] if groups else 'something'}"
                )
    ...
    def _extract_with_llm(
        self, speaker: str, content: str, session_id: str, session_date: str, session_idx: int
    ) -> list[MemCell]:
